...
This project uses Grafana's **HTML Graphics** panel for the animated course and **SQLite** data source for the imported workout. It runs inside a normal Grafana dashboard. The panel's HTML, CSS, and JavaScript are included in the dashboard export; no separate web app is required.
...
- Dashboard checks: executes the actual queries A–D before saving. Empty successful heart-rate results are valid when the workout contains no heart-rate samples.
...
Use the same database path for later imports. After replacing the demo or importing the real race with the importer's `--replace` option, refresh the dashboard to read the new data. If SQLite still holds a connection to the old file after atomic replacement, reconnect the data source or restart the Grafana service/container, then refresh again. A rebuild and reinstall are only needed when the dashboard source changes.
